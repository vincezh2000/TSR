...
class RankLSTM:
    def __init__(self, data_path, market_name, tickers_fname, parameters,
        ...
        # 读取股票名称，如果在NASDAQ里，就读取NASDAQ的name
        ...
        # 提取出的股票数据使用load_EOD_data（具体操作见函数内部）
        ...
        self.steps = steps
        self.epochs = epochs
        # 默认为股票代码总数，每一天输入全部股票数目
        if batch_size is None:
            self.batch_size = len(self.tickers)
        else:
            self.batch_size = batch_size
        # 0-755 train 755-1007 valid 1008- test
    ...
    def get_batch(self, offset=None):
        ...
        # (len_code, seq_len) 滑动窗口内的mask
        mask_batch = self.mask_data[:, offset: offset + seq_len + self.steps]
        # (len_code, 1) ，如果窗口内某一天的数据缺失了，该股票mask就为0
    ...
    def train(self):
        ...
        with tf.device(device_name):
            ...
            # 只需要最后一个h
            seq_emb = outputs[:, -1, :]
            # One hidden layer 最后全连接层回归，输出prediction
            ...
            # 如果数据缺失，损失就不计入--weights=mask
            # 此处mask起作用，不计算缺失的未知损失值
            reg_loss = tf.losses.mean_squared_error(
                ground_truth, return_ratio, weights=mask
            )
            # 预测正负（i股票收益率是否高于j股票，在第t天）
            # [len_code, 1] * [len_code, 1]^T = [len_code, len_code]
